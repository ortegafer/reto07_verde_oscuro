...
def calcular_primas_jubilacion(salario, edad, años_trabajados, interes1, interes2, 
                               duración_interes1, interes_rendimiento1, interes_rendimiento2, 
                               duración_interés_rendimiento1, fecha_jubilación=None):
    ...
    if fecha_jubilación is None:
        ...
        años_hasta_jubilacion = modelo.predict(X_pred)[0]  
        ...
        años_hasta_jubilacion = (fecha_jubilacion - dt.date(2025, 1, 1)).days / 365
    ...
    # 🔹 Función para validar y convertir a `float`
    def convertir_a_float(valor, nombre_variable):
        if isinstance(valor, list):  
            valor = valor[0]
        try:
            return float(valor)
        except ValueError:
            raise TypeError(f"{nombre_variable} debe ser un número, pero se recibió: {valor}")

    # 🔹 Validar y convertir valores a `float`
    interes1 = convertir_a_float(interes1, "interes1")
    interes2 = convertir_a_float(interes2, "interes2")
    interes_rendimiento1 = convertir_a_float(interes_rendimiento1, "interes_rendimiento1")
    interes_rendimiento2 = convertir_a_float(interes_rendimiento2, "interes_rendimiento2")

    # 🔹 Validar duración_interes1 y duración_interés_rendimiento1
    if isinstance(duración_interes1, list) and len(duración_interes1) >= 2:
        año1, mes1 = duración_interes1[:2]
    else:
        raise TypeError(f"duración_interes1 debe ser una lista con dos valores, pero se recibió: {duración_interes1}")

    if isinstance(duración_interés_rendimiento1, list) and len(duración_interés_rendimiento1) >= 2:
        año2, mes2 = duración_interés_rendimiento1[:2]
    else:
        raise TypeError(f"duración_interés_rendimiento1 debe ser una lista con dos valores, pero se recibió: {duración_interés_rendimiento1}")

    # 🔹 Cálculo de intereses (corregido)
    ...
    for i in range(0, len(fechas_formateadas) - (año2 * 12 + mes2)):
